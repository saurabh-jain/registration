	...
	do k = 1, num_nodes, 1
	  ...
	  do l=1, num_nodes, 1
		...
		dz = dz - sfactor*Kv_diff*alpha(l)*zdz*2* &
				  x_diff - Kh_diff*alpha(l)*x_diff
		dm = dm + 0*sfactor*Kv*alpha(l)*zdz + Kh*alpha(l)
		kvz = 2*(x_diff(1)*z(l,1,t)+x_diff(2)*z(l,2,t)*x_diff(3)*z(l,3,t))
		dJ = dJ + sfactor*J(k,t)*alpha(l)*Kv_diff*kvz
...
num_times,num_nodes,dx,dm,dJ,ex,ez,em,eJ,ealpha)
  implicit none
  integer :: num_nodes, num_times, kvo, kho
  real(8) :: sfactor, dt
  real(8) :: kvs, khs
...
!f2py real(8), intent(out), dimension(num_nodes, num_times) :: eJ
!f2py real(8), intent(out), dimension(num_nodes, num_times) :: em
!f2py real(8), intent(out), dimension(num_nodes, 3, num_times) :: ez
  ...
  do t=num_times,2,-1
	do k=1,num_nodes,1
	  do l=1,num_nodes,1
		...
		x_diff = x(k,:,t)-x(l,:,t)
		x_diff_sqr = dot_product(x_diff,x_diff)
		...
		dex = dex + (-sfactor)*( &
				  ...
				  + sfactor*( alpha(l)*zdz*(Kv_diff2*4*x_diff_sqr+2*Kv_diff)*ez(k,:,t) &
				  + alpha(k)*zdz*(Kv_diff2*4*x_diff_sqr-2*Kv_diff)*ez(l,:,t)) &
				  + alpha(l)*(Kh_diff2*4*x_diff_sqr+2*Kh_diff)*ez(k,:,t) &
				  + alpha(k)*(Kh_diff2*4*x_diff_sqr-2*Kh_diff)*ez(l,:,t) & 
				  - alpha(l)*Kh_diff*2*x_diff*em(k,t) &
				  - alpha(k)*Kh_diff*2*x_diff*em(l,t) &
				  - sfactor*(J(k,t)*alpha(l)*(Kv_diff2*4*x_diff_sqr+2*Kv_diff)*z(l,:,t)*eJ(k,t) &
				  + J(l,t)*alpha(k)*(Kv_diff2*4*x_diff_sqr-2*Kv_diff)*z(k,:,t)*eJ(l,t))
	  end do
	end do
  end do
