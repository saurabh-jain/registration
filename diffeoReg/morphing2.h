...
class Morphing:public ImageMatching
{
  ...
  */
  ...
  double norm_increment_forward(const VectorMap &v, const Vector &Jcur, const Vector & Jnext) {
    ...
    Id.idMesh(v.d) ;
    if (param.matchDensities){
      divergence(v, divf, param.spaceRes) ;
      divf *= dt ;
      divf += 1 ;
    }
  ...
  public:
    ...
    double computeGrad(VectorMap &w, VectorMap& grad){
      ...
      double dt = 1.0/_sh->w.size() ;
      //cout << "in compute grad" << endl ;
      _sh->kernel(w, ft) ;
      if (_sh->param.matchDensities)
	divergence(ft, divf, _sh->param.spaceRes) ;
  ...
  */
  ...
  public:
    ...
    void computeLocalMaps() {
      ...
      Id.idMesh(_sh->w.d) ;
      if (_sh->param.matchDensities) {
	div1.resize(T, _sh->w.d) ;
	div2.resize(T, _sh->w.d) ;
      }
      for (unsigned int t=0; t<T; t++) {
	...
	if (_sh->param.matchDensities) {
	  divergence(f, div1[t], _sh->param.spaceRes) ;
	  div2[t].copy(div1[t]) ;
	  div1[t] *= dt ;
	  div2[t] *=-dt ;
	  div1[t] += 1 ;
  ...
  void smoothIConj(int nbStep, _real error)
  {
    ...
    if (param.matchDensities) {
	  divergence(f, divf1, param.spaceRes) ;
	  divf2.copy(divf1) ;
	  divf1 *= dt ;
	  divf2 *=-dt ;
	  divf1 += 1 ;
